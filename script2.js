...
function formatAndHighlight(searchTerm) {
    const filteredData = data.filter(item => JSON.stringify(item).includes(searchTerm));
    const formattedData = JSON.stringify(filteredData, null, 2)
        .replace(new RegExp(searchTerm,  'g'), `<span class="highlight">${searchTerm}</span>`);
    return formattedData;
}

function updateResult() {
    const searchTerm = searchInput.value.trim();  
    const formattedResult = formatAndHighlight(searchTerm);
    result.innerHTML = `<pre>${formattedResult}</pre>`;
}

searchInput.addEventListener('input', updateResult);
